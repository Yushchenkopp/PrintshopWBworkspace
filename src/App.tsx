...
import { generateCollageTemplate, updateCollageHeader, updateCollageFilters, type TemplateType } from './utils/TemplateGenerators';
import { transliterate } from './utils/Transliteration';
// Импорт экспорта оставляем, но если файла нет - закомментируй
...
function App() {
  ...
  // const [parent] = useAutoAnimate(); // Removed auto-animate in favor of dnd-kit
  ...
  const prevPropsRef = React.useRef({
    imagesJson: '[]',
    template: 'collage' as TemplateType,
    aspectRatio: 1,
    isBWEnabled: true,
    brightness: 0
  });

  const lastDateRef = React.useRef('05.09.2025');
        ...
        try {
          if (template === 'collage') {
            ...
            const isStructureChanged =
              ...
              template !== prevPropsRef.current.template ||
  ...
  }, [canvas, template, images, aspectRatio, debouncedHeaderText, footerName, footerDate, isBWEnabled, isSinceEnabled, textColor, brightness]);
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button
                ...
                className={`aspect-square flex items-center justify-center rounded-xl cursor-pointer group relative transition-all duration-200 ease-out ${template === item.id ? 'bg-zinc-900 text-white shadow-md' : 'bg-white/40 border border-zinc-900/10 shadow-sm text-zinc-600 hover:scale-105 hover:bg-white/80 hover:shadow-md hover:border-zinc-300 hover:text-zinc-900'}`}
                title={item.label}
              >
